...
Research / Auto can hire the research package for search → fetch → vault ingest, and still honour the **workspace** model allowlist. A missing local CE tree no longer hides an already-installed global CE skill (read/execute only; setup flags still gate installation).
...
Skill-shell rationalization is an **import-charter behavior/performance gate**,
not source parity: Switchbay and okbay import the CE, curiosity-merge, and
okstratr skills through public contracts. They do not vendor, merge, copy, or
re-implement those servers for a shell. The gate pins CE skill
`5d2b967558dd052c62b23fbc03217b1ac2a18d5a` and uses BioCure wiki fixture
`biocure-confirm-v1-query-5b9711895` at freeze tip `5b9711895`; the latter is a
wiki/corpus pin, not the CE skill SHA. See the [import-boundary matrix](../handoff/2026-09-20-skill-shell-parity.md).
